...
- **Lab management tool**: cluster management, AI endpoints, and infrastructure monitoring. It's now running in production in the lab.
...
- **claude-perfmon on WSL**: adapting a Claude usage monitoring tool (originally for Mac) to run on WSL, with the changes contributed back to the original project via a Pull Request on GitHub.
- **Presentation for NGO Rescate**: a PowerPoint on using AI in job searching, generated from a script I wrote drawing from several sources — the most valuable being notes from my former boss and good friend Francisco Javier.
